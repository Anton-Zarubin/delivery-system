...
public class WalletControllerTest {
    ...
    void createWalletWithException() throws Exception {
        doThrow(new AlreadyExistsException("An e-wallet for user with ID {0} already exists"))
    ...
    void replenishBalance() throws Exception {
        ReplenishmentDto replenishmentDto = new ReplenishmentDto(BigDecimal.TEN);

        when(walletService.replenishBalance(1L, replenishmentDto)).thenReturn(BigDecimal.TEN);
        mockMvc.perform(
                        patch("/wallet/balance/replenish")
                                ...
                                .accept(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(replenishmentDto))
                                .contentType(MediaType.APPLICATION_JSON)
                )
    ...
    void replenishBalanceWithException() throws Exception {
        ...
        doThrow(new ResourceNotFoundException("The wallet for user with ID 1 not found"))
                .when(walletService).replenishBalance(1L, replenishmentDto);
        mockMvc.perform(
                        patch("/wallet/balance/replenish")
                                .with(request -> {
                                    request.addHeader("id", 1L);
