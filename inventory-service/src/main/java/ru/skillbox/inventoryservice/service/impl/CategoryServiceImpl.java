...
public class CategoryServiceImpl implements CategoryService {
    ...
    return categoryRepository.findById(id)
            .orElseThrow(() ->
                    new ResourceNotFoundException(MessageFormat.format("Category with id {0} not found", id)));
    }
    ...
    public Category getByTitle(String title){
        return categoryRepository.findByTitle(title)
                .orElseThrow(() ->
                        new ResourceNotFoundException(MessageFormat.format("Category with title {0} not found", title)));
    }
